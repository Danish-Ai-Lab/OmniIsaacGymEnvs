...
class MoonbotTask(RLTask):
    ...
    def update_config(self, sim_config):
        ...
        ## Set initial condition of robot
        self.initial_position = torch.tensor([0.0, 0.0, 0.5], dtype=torch.float32)
        self.initial_orientation = torch.tensor([1, 0, 0, 0], dtype=torch.float32)

    def set_up_scene(self, scene) -> None:
        self.get_moonbot()
        super().set_up_scene(scene)
        self._moonbot = ArticulationView(prim_paths_expr="/World/envs/.*/moonbot", name="moonbotview")
        scene.add(self._moonbot)
        return

    def get_moonbot(self):
        moonbot = Moonbot(
            prim_path=self.default_zero_env_path + "/moonbot", 
            name="Moonbot", 
            usd_path="models/moonbot.usd",
            translation=self.initial_position, 
            orientation=self.initial_orientation,
        )
        self._sim_config.apply_articulation_settings(
            "Moonbot", get_prim_at_path(moonbot.prim_path), self._sim_config.parse_actor_config("Moonbot")
        )

    def get_observations(self) -> dict:
        torso_position, torso_rotation = self._moonbotsimple6s.get_world_poses(clone=False)
        root_velocities = self._moonbotsimple6s.get_velocities(clone=False)
        dof_pos = self._moonbotsimple6s.get_joint_positions(clone=False)
        dof_vel = self._moonbotsimple6s.get_joint_velocities(clone=False)
        ...
        observations = {self._moonbotsimple6s.name: {"obs_buf": self.obs_buf}}
        return observations
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        self._moonbot.set_joint_position_targets(actions)

    def reset_idx(self, env_ids):
        num_resets = len(env_ids)
        # randomize DOF velocities
        velocities = torch_rand_float(-0.1, 0.1, (num_resets, self._moonbotsimple6s.num_dof), device=self._device)
        ...
        self._moonbot.set_joint_positions(dof_pos, indices)
        self._moonbot.set_joint_velocities(dof_vel, indices)

        self._moonbot.set_world_poses(
            self.initial_root_pos[env_ids].clone(), self.initial_root_rot[env_ids].clone(), indices
        )
        self._moonbot.set_velocities(root_vel, indices)
    ...
    def post_reset(self):
        ...
        dof_names = self._moonbotsimple6s.dof_names
        for i in range(self.num_actions):
            name = dof_names[i]
            angle = self.named_default_joint_angles[name]
            self.default_dof_pos[:, i] = angle

        self.initial_root_pos, self.initial_root_rot = self._moonbotsimple6s.get_world_poses()
        self.current_targets = self.default_dof_pos.clone()

        dof_limits = self._moonbotsimple6s.get_dof_limits()
        self.moonbotsimple6_dof_lower_limits = dof_limits[0, :, 0].to(device=self._device)
        self.moonbotsimple6_dof_upper_limits = dof_limits[0, :, 1].to(device=self._device)
        ...
        # randomize all envs
        indices = torch.arange(self._moonbot.count, dtype=torch.int64, device=self._device)
    ...
    def calculate_metrics(self) -> None:
        ...
        self.fallen_over = self._moonbotsimple6s.is_base_below_threshold(threshold=0.51, ground_heights=0.0)
